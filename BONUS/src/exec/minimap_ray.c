...
/*   Updated: 2025/04/18 22:56:28 by imatek           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	ft_draw_line_x(t_data *data, t_point *end, t_bresenham *rayon)
{
	...
	{
		if (rayon->pixel_x >= 0 && rayon->pixel_x < WIDTH && rayon->pixel_y >= 0 && rayon->pixel_y < HEIGHT)
			my_pixel_put(data->img, rayon->pixel_x, rayon->pixel_y, COLOR_PLAYER);
		if (d > 0)
		{
